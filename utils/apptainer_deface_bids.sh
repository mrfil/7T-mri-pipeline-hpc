...
export projDir="${base_dir}/${VERSION}/testing/${PROJECT}"
export IMAGEDIR="${base_dir}/apptainer_images"

CACHESING="${base_dir}/${VERSION}/scratch/scache/${PROJECT}_deface"
TMPSING="${base_dir}/${VERSION}/scratch/stmp/${PROJECT}_deface"
...
find . -type f -name "*_${imt[@]}.nii.gz" -exec sh -c 'echo "$1"' - {} \; -and -exec bash -c 'apptainer run --contain --no-home --cleanenv -B ${dataDir}:/data --pwd /data ${IMAGEDIR}/pydeface-v2.0.0.sif pydeface "$1"' - {} \; -and -print
