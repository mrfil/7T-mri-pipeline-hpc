...
subject=$1
sesname=$2
cd /shared/mrfil-data/pcamach2/CUPS/bids/derivatives/qsirecon/sub-CUPS005/ses-A/dwi
# cd /datanoddi
for atlas in aal116 aicha384 brainnetome246 gordon333 power264 schaefer100x17 schaefer100x7 schaefer200x17 schaefer200x7 schaefer400x17 schaefer400x7; do
    ...
    while read roi_name_tmp; do
        ...
        for metric in ICVF ISOVF OD; do
            ...
            fslstats ${metricImage} -k roitmp.nii.gz -M -S -R -r > roistats.txt
            roi_stats_tmp=`cat roistats.txt`
            echo $roi_stats_tmp
            echo "${metric}_${atlas}_${roi_name}_mean_nonzero ${metric}_${atlas}_${roi_name}_standard_deviation ${metric}_${atlas}_${roi_name}_min ${metric}_${atlas}_${roi_name}_max ${metric}_${atlas}_${roi_name}_min_robust ${metric}_${atlas}_${roi_name}_max_robust" > roistats.csv
